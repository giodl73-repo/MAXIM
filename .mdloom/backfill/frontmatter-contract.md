...
Frontmatter is source-corpus metadata introduced during MDLOOM backfill, one
module at a time. It exists to make MAXIM queryable without breaking the
human-readable library layout.

MDLOOM rendered output should not include frontmatter by default. MDPORT output
...
| `source-record` | A backsource/provenance note under `.mdloom/backfill/sources/`. | `.mdloom/backfill/sources/computing-software/...` |
| `generated-pack` | Generated CROP/MDPORT/FLETCH artifact metadata. | `.crop/packs/...` |
...
| `backsource_ids` | Stable IDs from `.mdloom/backfill/sources/<module>/`. |
...
| Downstream fact pack | Verified guides plus source records, emitted as MDPORT and fetched with FLETCH. |

## MDLOOM output policy
...
| MDPORT/FLETCH artifacts | Preserve frontmatter fields as MDPORT metadata and FLETCH-facing artifact metadata. |

CROP `664b44f` and MDPORT `d87afe6` provide the first concrete path: Markdown
frontmatter is stripped from packed text and copied into each emitted MDPORT
section's metadata map.

## Backfill rule

Do not add frontmatter to the whole repo at once. Add it to a module only when
that module is selected for MDLOOM backfill and source-custody review.
