...
backsource_ids: [mdloom-backfill:combinatorics:09-extremal-and-design, git-history:combinatorics:09-extremal-and-design]
...
```
   ...
   MDLOOM (symmetric chain decomposition / LYM inequality):
        SUM over an antichain F of  1 / C(n, |A|)  <= 1   (LYM)
        => |F| <= C(n, n/2) since the middle binomial is largest.
```

The **LYM inequality** (Lubell–Yamamoto–Meshalkin) is the slick mdloom: each set
