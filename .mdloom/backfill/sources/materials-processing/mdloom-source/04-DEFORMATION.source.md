...
proof_original: "04-DEFORMATION.md"
---
---
...
backsource_ids: [mdloom-backfill:materials-processing:04-deformation, git-history:materials-processing:04-deformation]
...
**Hall-Petch relation — the key microstructure-to-property bridge**: Yield strength scales as σ = σ₀ + k/√d, where d is grain size. The √d denominator arises from dislocation pile-up theory: dislocations accumulate at grain boundaries, and the stress concentration at a pile-up of length d scales as √d — the same square-root-of-length scaling that appears in fracture mechanics (K = σ√(πa), see 05-FRACTURE-MECHANICS.md). Refining grain size from 100 μm to 10 μm increases yield strength by k × (1/√10 - 1/√100) ≈ 2.5k. This is the single most important quantitative link between processing (which controls grain size) and mechanical properties.
