...
proof_original: "09-CHARACTERIZATION.md"
---
---
...
backsource_ids: [mdloom-backfill:materials-processing:09-characterization, git-history:materials-processing:09-characterization]
...
**Signal processing bridge:** Bragg's law (nλ = 2d sinθ) is constructive interference — the same physics as diffraction gratings, antenna arrays, and phased array radar. The XRD diffraction pattern is a fingerprint of the crystal structure: each phase produces a unique set of d-spacings. Phase identification from XRD is pattern matching against a database (ICDD Powder Diffraction File) — the same operation as hash lookup or signature matching.
...
**NDE probability of detection (POD) vs detection limit** — POD curves are ROC curves for physical defect classifiers: P(detect | flaw size a) vs a is the TPR as a function of signal strength, with the 90/95 value serving as the operating point. The false-negative (miss a crack) vs false-positive (unnecessary repair) tradeoff maps exactly to ROC analysis in ML classifier evaluation.: An NDE technique's nominal detection limit (e.g., "UT detects 0.5 mm flaws") is deterministic. Real POD curves are probabilistic — there's a statistical probability of detecting a given flaw size, not a sharp cutoff. ASTM E2862 and MIL-HDBK-1823A define POD methodology. For damage tolerance calculations, use the 90/95 POD value (90% probability of detection with 95% confidence), not the nominal limit.
