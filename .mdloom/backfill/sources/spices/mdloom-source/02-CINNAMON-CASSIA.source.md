---
tags: [backfill]
ops: [backfill]
content_tags: [markdown]
proof_original: "02-CINNAMON-CASSIA.md"
---
---
maxim_schema: maxim.frontmatter.v1
id: maxim:spices:cinnamon-cassia
kind: guide
module: spices
section: spices
title: Cinnamon and Cassia - The Bark Spices
status: source-custody
source_custody: partial
current_path: spices/02-CINNAMON-CASSIA.md
canonical_path: spices/02-CINNAMON-CASSIA.md
backsource_ids: [mdloom-backfill:spices:02-cinnamon-cassia, git-history:spices:02-cinnamon-cassia]
concepts: [cinnamon, cassia]
root_concepts: [cinnamon, cassia]
index_roles: [guide, root-concept]
remap_from: []
remap_to: []
updated: null
---
# Cinnamon & Cassia — The Bark Spices

## The Big Picture

```
+------------------------------------------------------------------+
|              THE CINNAMON IDENTITY PROBLEM                       |
|                                                                  |
|  "CINNAMON" sold globally = two different products               |
|                                                                  |
|  CEYLON / TRUE CINNAMON          CASSIA                          |
|  (Cinnamomum verum)              (C. cassia, C. aromaticum,      |
|  Sri Lanka primary source        C. burmannii, C. loureiroi)     |
|                                                                  |
|  Thin, papery, tight scroll      Thick, single curl, rough bark  |
|  Delicate, complex flavor        Strong, sweet, simpler flavor   |
|  Low coumarin (~250 ppm)         High coumarin (~2,100 ppm)      |
|  Higher price                    Lower price - dominates US mkt  |
|                                                                  |
|  What you think "cinnamon" smells like = cassia.                 |
|  Ceylon has a subtler, more floral profile.                      |
+------------------------------------------------------------------+
```

---

## Botanical Reality

Both are *Cinnamomum* (Lauraceae family). The spice is the **dried inner bark** — not the woody outer bark, but the inner cambium layer scraped off, rolled, and dried.

```
BARK HARVESTING PROCESS:
  +--------------------------------------------------+
  |                                                  |
  | 1. Cut young branches (2-3 yr old, pencil thick) |
  | 2. Scrape outer bark (rough, discarded)          |
  | 3. Peel inner bark in long strips                |
  | 4. Strips curl as they dry                       |
  |                                                  |
  | CEYLON RESULT:          CASSIA RESULT:           |
  | Multiple thin layers    Single thick curl        |
  | stack together          from one side only       |
  | -> tight cigar     -> loose U shape              |
  | Like layers of paper    Like a single roll       |
  +--------------------------------------------------+

Visual ID test:
  Tight multiple-layer scroll + tan/light brown = Ceylon
  Single thick curl + dark reddish-brown = cassia
  Ground powder: cannot distinguish visually -> need coumarin test
```

---

## Species Comparison

| Feature | Ceylon (*C. verum*) | Vietnamese (*C. loureiroi*) | Indonesian (*C. burmannii*) | Chinese (*C. cassia/aromaticum*) |
|---------|--------------------|-----------------------------|-----------------------------|---------------------------------|
| Common name | True cinnamon, Sri Lanka cinnamon | Saigon/Vietnamese cinnamon | Korintje, padang | Chinese cassia |
| Cinnamaldehyde | ~55–65% of EO | ~85–95% of EO (highest) | ~75–85% | ~70–80% |
| Coumarin | ~250 ppm | ~6,000 ppm | ~2,000 ppm | ~2,100 ppm |
| Flavor profile | Delicate, floral, complex | Very intense, sweet, spicy | Milder, slightly earthy | Similar to Vietnamese, milder |
| US market share | ~5% | ~40% | ~50% | ~5% |
| Price point | Highest | Medium-high | Lowest | Low |

---

## Key Chemistry

### Cinnamaldehyde — Primary Compound

```
CINNAMALDEHYDE (C9H8O):
  Trans-cinnamaldehyde = primary volatile in all species
  Phenylpropanoid pathway: phenylalanine -> cinnamic acid -> cinnamaldehyde

  Receptor binding:
  - TRPA1 agonist (chemical irritant receptor) -> mild burning sensation
  - Low-level antibacterial and antifungal properties
  - Inhibits NF-κB pathway (anti-inflammatory in cell studies)
  - Active ingredient in commercial cassia-based antimicrobial coatings

  ~90% of essential oil in cassia species
  ~55% in Ceylon (more complex profile = more other compounds)
```

### Coumarin — The Safety Concern

```
COUMARIN (C9H6O2):
  Benzopyranone structure; natural secondary metabolite
  NOT the same as warfarin (4-hydroxycoumarin anticoagulant)
    - common confusion, different class

  CEYLON:      ~250 ppm (parts per million by dry weight)
  CASSIA:      ~1,000-6,000 ppm depending on species/grade

  Toxicity:
  - Hepatotoxic in rats (CYP2A6/2A13 metabolism -> hepatotoxic metabolite)
  - Human hepatotoxicity documented in case reports at supplement doses
  - European Food Safety Authority (EFSA) TDI: 0.1 mg/kg body weight/day
  - EU food regulations limit coumarin in foods using cassia
  - Germany (2006): advisory to limit cassia-heavy foods (lebkuchen)

  Practical implication:
  - Culinary use (occasional): negligible risk
  - Daily cinnamon supplements using cassia: potential hepatotoxic range
  - True Ceylon: well below EFSA TDI even with heavy culinary use
```

### Eugenol

```
EUGENOL (phenylpropanoid, also in cloves/bay):
  ~5-10% in Ceylon essential oil vs ~trace in cassia
  Anesthetic + TRPV1 agonist
  Higher eugenol = more of Ceylon's complexity
  Lower eugenol in cassia = simpler, cassia-specific flavor
```

---

## Colonial History — Sri Lanka

```
TIMELINE OF CEYLON CINNAMON MONOPOLY:

1200s: Arab traders control Ceylon cinnamon supply
        Fabricated origin myths to protect source locations
        (Herodotus: "cinnamon birds bring it from distant nests")

1505: Portuguese arrive in Ceylon (modern Sri Lanka)
      First Europeans to locate true cinnamon source
      Establish trading relationships, then coercion

1638: VOC (Dutch East India Company) allied with Kingdom of Kandy
      Against Portuguese

1658: Dutch seize Portuguese Ceylon
      -> VOC establishes strict cinnamon monopoly
      -> Cinnamon gardens (kurundu) under Dutch control
      -> Punishment for unauthorized cinnamon harvesting: DEATH
      -> VOC employed Salagama caste as hereditary peelers
      -> Required to produce fixed annual quantity

1796: British take Ceylon from Dutch (Napoleonic Wars)
      -> British EIC - less focus on cinnamon monopoly
      -> Cassia from China/Indonesia already flooding markets
      -> Ceylon cinnamon monopoly effectively over

1833: Ceylon crown colony - cinnamon becomes one crop among many
      Subsequent replacement by tea as primary export

Present: Sri Lanka still produces ~85-90% of world's Ceylon cinnamon
         Premium PDO product, price 5-10x cassia
```

---

## Ancient Cinnamon — The Source Mystery

The ancient world had cinnamon but couldn't explain where it came from:

```
HISTORICAL MYSTERY:
  Cinnamon mentioned in:
  - Chinese writing (2700 BCE) -> clearly cassia from China
  - Egyptian embalming (1400s BCE) -> which species?
  - Hebrew Bible (Exodus, Song of Songs) -> 'qinnamon'
  - Ancient Greek/Roman texts

  The puzzle:
  Ceylon cinnamon grows ONLY in Sri Lanka
  Ancient Egypt/Rome had no sea route to Sri Lanka
  Arab traders controlled the supply and DELIBERATELY obscured origin

  The myth (Herodotus/Aristotle):
  "Cinnamon was carried by giant birds to build their nests.
   Collectors frightened the birds with heavy weights,
   causing nests to fall."
  -> A propaganda story invented to keep competitors away

  Modern scholarship:
  Ancient "cinnamon" probably = East African/Arabian cassia species
  Ceylon cinnamon may have entered trade later than assumed
  DNA analysis of ancient spice residues ongoing
```

---

## Culinary Geography

```
REGION -> PREFERRED "CINNAMON" TYPE:

  Mexico / Latin America:  Ceylon (C. verum)
    -> Mexican hot chocolate, churros, rice pudding
    -> Cultural connection from colonial Spanish import routes

  North America (US/Canada): Cassia (C. burmannii or loureiroi)
    -> "Cinnamon" = what you grew up with = cassia
    -> Apple pie, oatmeal, cereal marketing flavor profile

  United Kingdom: Ceylon traditionally preferred
    -> Commonwealth connection to Sri Lanka

  Sri Lanka / India: Both, by use case

  China: Cassia (C. cassia) - indigenous crop
    -> Five Spice Powder uses cassia

  The irony: The "true" version is actually the regional variant
  for most US consumers - cassia is what they know as "cinnamon"
```

---

## Decision Cheat Sheet

| Question | Answer |
|----------|--------|
| How do I know if I bought Ceylon vs cassia? | Physical: multiple thin layers = Ceylon; single thick curl = cassia. Chemical: coumarin content by GC-MS |
| Is cinnamon safe in large quantities? | Depends on species. Ceylon: yes. Cassia: coumarin limits apply — supplement doses problematic, culinary use fine |
| What's the flavor difference? | Ceylon = delicate, floral, complex. Cassia (especially Vietnamese) = intense, sweet, "cinnamon candy" profile |
| Why does US cinnamon taste different from Mexican cinnamon? | US = cassia; Mexico = Ceylon |
| What's cinnamaldehyde's mechanism? | TRPA1 agonist → mild burning/warming. Also antibacterial in vitro |
| Is coumarin the same as warfarin? | No. Coumarin = benzopyranone. Warfarin = 4-hydroxycoumarin anticoagulant. Different structures, different mechanisms. |

---

## Common Confusion Points

**Coumarin ≠ warfarin/coumadin:** Warfarin is a 4-hydroxycoumarin derivative (anticoagulant). Coumarin itself (in cassia) is metabolized to hepatotoxic metabolites in some individuals via CYP2A6. Different compound, different mechanism, both concerning at high doses but completely different clinical effects.

**"Cinnamon reduces blood sugar" claims:** Small studies exist but results are mixed and predominantly from cassia. The coumarin content of cassia makes it a risky supplement vehicle. Ceylon is safer if this is pursued.

**Vietnamese cinnamon being "better":** Vietnamese (Saigon) cassia has the highest cinnamaldehyde content (~85–95%) — most intense flavor. But highest coumarin too (~6,000 ppm). "Best" depends on what you want: flavor intensity or safety margin.

**The "cinnamon stick" in your Indian biryani:** Usually cassia (cheaper, locally produced in Indonesia/India), not Ceylon. The whole bark is for flavor infusion during cooking, removed before serving.
