...
proof_original: "07-LIE-GROUPS.md"
---
---
...
backsource_ids: [mdloom-backfill:differential-geometry:07-lie-groups, git-history:differential-geometry:07-lie-groups]
concepts: [lie, groups]
...
```
**Sym+(n) and natural gradient**: The Fisher information matrix F(θ) of a parametric model is an element of Sym+(n), making the parameter space a Riemannian manifold with metric g = F. Natural gradient descent θ_{t+1} = θ_t − η F(θ_t)^{-1} ∇L(θ_t) is exactly Riemannian gradient descent on this statistical manifold. The log-Euclidean metric d_LE(A,B) = ||log(A) − log(B)||_F approximates the affine-invariant metric at lower computational cost (O(n^3) for matrix log vs. O(n^3) for the full exp/log on Sym+(n)) and is used in practical frameworks (Geomstats, pyRiemann, geoopt).
...
**Equivariant networks as representation theory**: The Peter-Weyl theorem (compact Lie group G: L^2(G) = ⊕_ρ dim(ρ) · ρ, direct sum over irreducible representations) is the foundation for equivariant neural networks. For SO(3): irreps are the spherical harmonics Y_l^m of degree l. NequIP, SEGNN, and e3nn architectures use SO(3) irreps as feature types: a "type-l feature" transforms as Y_l under rotations. Network layers must be intertwiners (linear maps commuting with the group action): the Clebsch-Gordan decomposition l₁ ⊗ l₂ = |l₁−l₂| ⊕ ... ⊕ (l₁+l₂) determines which tensor products of features are allowed. Equivariance is enforced by construction — the network's output transforms correctly under any rotation because each layer respects the representation theory. This abstract framework (representation theory of Lie groups → allowed linear maps between feature spaces) is the mathematical backbone of all equivariant architectures.
