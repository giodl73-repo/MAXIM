...
proof_original: "06-CURVATURE.md"
---
---
...
backsource_ids: [mdloom-backfill:differential-geometry:06-curvature, git-history:differential-geometry:06-curvature]
concepts: [curvature]
...
```
**Curvature in ML and network science**: (1) **Ollivier-Ricci curvature on graphs**: a discrete analogue of Ricci curvature for edges of a graph — positive curvature indicates tightly-connected clusters, negative curvature indicates bottlenecks between communities. Used for community detection and graph surgery (removing negatively curved edges partitions the graph). Forman-Ricci curvature on edges provides a computationally cheaper alternative. (2) **Loss landscape curvature**: positive sectional curvature in the parameter-space Riemannian geometry (with the Fisher metric) correlates with saddle-point problems; negative curvature directions are escape routes exploited by second-order methods (Newton, cubic regularization). (3) **Normalized Ricci flow on surfaces**: ∂g/∂t = −2(Ric − (r/n)g) converges to constant curvature metric, providing the 2D uniformization theorem as a flow fixed point: χ > 0 → sphere, χ = 0 → flat torus, χ < 0 → hyperbolic surface. Perelman used Ricci flow (with surgery) to prove the 3D Poincare conjecture.
...
**Atiyah-Singer index theorem — the curvature → topology → analysis capstone**: For an elliptic operator D on compact M: index(D) = dim ker(D) − dim coker(D) = ∫_M [topological characteristic classes]. Key instances: D = de Rham d + d* → index = χ(M) (Gauss-Bonnet); D = Dirac operator on spin manifold → index = Â(M) (Â-genus, built from Pontryagin classes of TM); D = Dolbeault ∂̄ + ∂̄* on complex manifold → index = χ(O_M) = ∫ Td(TM_C) (Hirzebruch-Riemann-Roch). Physics application: the chiral anomaly in QFT = index of the Dirac operator coupled to a gauge field. The number of left-handed minus right-handed zero modes of the Dirac equation is a topological invariant — it cannot change under continuous deformation of the gauge field.
