...
proof_original: "05-CONNECTIONS.md"
---
---
...
backsource_ids: [mdloom-backfill:differential-geometry:05-connections, git-history:differential-geometry:05-connections]
...
```
**Berger classification — physics/geometry payoff**: Hol = SU(n) → Calabi-Yau manifold (Ricci-flat Kähler, relevant to string theory compactification on CY_3); Hol = Sp(n) → hyper-Kähler manifold (supersymmetric sigma models); Hol = G_2 → exceptional holonomy in 7D (M-theory compactification); Hol = Spin(7) → exceptional 8D geometry. Each restricted holonomy group corresponds to a specific geometric structure and a specific class of physical theories.

**Parallel transport ↔ transformer attention**: Parallel transport of a vector v along a path γ is path-dependent — the transported vector depends on which route you take (non-zero curvature). Transformer self-attention computes a weighted combination of value vectors using query-key scores: Attention(Q,K,V) = softmax(QK^T/√d)V. This is a learnable transport operator: the "path" is the sequence of tokens, the "transport" moves information from position j to position i, and the attention weights are the connection coefficients. Non-trivial attention patterns (where context at position i affects the representation at position j differently depending on the attention path) have the structure of a connection with non-zero curvature. Papers on geometric attention (Hutchinson et al. 2021) formalize this: attention heads = connection coefficients on a sequence manifold. The curvature measures how much the transported information depends on the context — exactly what attention is designed to capture.
...
**ML application: message-passing GNNs as parallel transport on graphs**. A connection on a vector bundle over a graph defines how to transport feature vectors along edges. Message-passing GNNs compute exactly this: aggregating neighbor features is parallel transport of feature vectors from neighboring nodes to the central node. In gauge-equivariant CNNs (Cohen & Welling 2019): feature maps are sections of a G-equivariant vector bundle, convolution kernels are parallel transport operators (connection), and gauge symmetry = freedom to choose local trivializations (coordinate frames at each node). The "gauge" in gauge-equivariant networks is literal — it is the mathematical gauge freedom of fiber bundle theory applied to neural network feature spaces.
