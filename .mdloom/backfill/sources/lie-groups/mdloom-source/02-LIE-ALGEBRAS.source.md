...
proof_original: "02-LIE-ALGEBRAS.md"
---
---
...
backsource_ids: [mdloom-backfill:lie-groups:02-lie-algebras, git-history:lie-groups:02-lie-algebras]
...
- `lie-groups/01-MATRIX-GROUPS.md` — matrix groups whose tangent spaces become Lie algebras.
- `abstract-algebra/02-SUBGROUPS-QUOTIENTS.md` — bracket structure as algebraic linearization.
- `differential-geometry/02-TANGENT-BUNDLES.md` — tangent spaces and vector fields as geometric substrate.

## Decision Cheat Sheet

| If you need to diagnose... | Start With | Key Caveat |
|---|---|---|
| Lie algebra of a group | Tangent space at identity `T_e G` | Determines local structure, not global topology of `G`. |
| Non-commutativity | Bracket `[X,Y]`, matrix commutator for matrix groups | Abstract Lie brackets need not be presented as matrices. |
| Exponential map | `exp: g -> G` from infinitesimal to finite transformations | Not globally surjective for many non-compact groups. |
| One-parameter subgroups | `phi_X(t) = exp(tX)` | Captures flows generated by one algebra element only. |
| BCH multiplication | `log(exp(X)exp(Y)) = X + Y + 1/2[X,Y] + ...` | Converges locally near zero, not as a global multiplication chart. |
| Adjoint representation | Conjugation action `Ad_g(X) = gXg^{-1}` | Differentiating gives `ad_X(Y) = [X,Y]`. |
| Semisimplicity test | Killing form `B(X,Y) = tr(ad_X ad_Y)` | Nondegenerate means semisimple; simple is stronger. |
