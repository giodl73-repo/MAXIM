...
proof_original: "07-MARINE-APPLICATIONS.md"
---
---
...
backsource_ids: [mdloom-backfill:rope-cordage:07-marine-applications, git-history:rope-cordage:07-marine-applications]
...
A sailing vessel's rope/wire system divides into two categories with different structural requirements: standing rigging (holds the mast static) and running rigging (controls sails and adjusts dynamically). These require opposite mechanical properties.

```
MARINE RIGGING OVERVIEW
========================
  ...
  │  │  Lower shrouds (lower mast support)                        │ │
  │  │  Stays (intermediate support)                              │ │
  │  │  Material: Stainless wire, rod, or high-modulus rope       │ │
  │  │  Key property: MINIMUM ELONGATION (mast alignment)        │ │
  │  └────────────────────────────────────────────────────────────┘ │
  │                                                                   │
  │  RUNNING RIGGING (movable, operational):                        │
  │  ┌────────────────────────────────────────────────────────────┐ │
  ...
  DECK HARDWARE BRIDGE:
  Winches, cleats, blocks, and fairleads translate between these
  two systems. The line system must be compatible with hardware
  (diameter matches winch groove; stiffness allows cleating).
```
...
================================
  ...
  │  └──────────────────────────────────────────────────────────┘   │
  │  Torque: Minimal (no outer strand twist in 1×19)               │
  │  Elongation: ~0.5% at 20% MBS (nearly rigid)                   │
  │  Bend radius: POOR — must terminate in swaged end; not for      │
  │    running through blocks                                       │
  │  Inspection: Broken wires on outer layer (surface cracks)      │
  │              "Meat hooks" — broken wire ends that cut crew      │
  └──────────────────────────────────────────────────────────────────┘
...
Mooring lines attach vessel to dock. The key property: **controlled elongation to absorb surge and wave action** while maintaining vessel position.
...
The anchor rode is the connection between vessel and anchor on the sea floor. It is the most dynamically loaded component of the anchoring system.
...
```
COILING (for storage and running):
====================================
...
| Marine Situation | Rope/Wire Choice |
|------------------|-----------------|
| Permanent stays/shrouds, budget | 1×19 stainless wire, swaged ends |
| Permanent stays, low stretch performance | Rod rigging or Dyneema |
| Halyards, non-racing | Polyester double braid (3/8"–1/2") |
| Halyards, racing — minimize stretch | UHMWPE/polyester composite braid |
| Sheets, all purposes | Polyester double braid |
| Mooring lines, absorb surge | Nylon 3-strand or double braid |
| Mooring lines, minimize stretch | Polyester double braid |
| Anchor rode, all situations | 15–30 ft chain + nylon 3-strand |
| Anchor rode, rocky bottom | Increase chain length (50+ ft) |
| Anchor rode, light vessel, light use | All nylon, 5:1 scope |

---

## Common Confusion Points

**Running rigging and standing rigging require opposite properties.** Standing rigging: must NOT stretch (or the mast moves). Running rigging: must pass through sheaves and be hand-worked (needs flexibility and compliance in bending). A wire stay on a halyard would be dangerous; a braided halyard as a shroud would let the mast pump dangerously.

**Scope is critical — 3:1 is not enough for rope rodes.** Chain at 3:1 works because the catenary (sag) absorbs surge. Rope at 3:1 goes straight to the anchor with minimal angle — anchor will not hold in any wind. Rope rodes require 5:1 minimum, 7:1 in strong conditions.

**Nylon mooring lines need regular inspection.** UV, abrasion at chocks, and cyclic loading degrade nylon without obvious external indication. Inspect annually at minimum; replace at first sign of hardening, stiffness, core exposure, or significant color change. In tropical/high-UV environments, replace every 3–5 years regardless of appearance.

**UHMWPE is too slippery for cleats without polyester cover.** Pure Dyneema rope will slip off cleats and through clutches. All running rigging using UHMWPE core must have a polyester or nylon braided cover that provides the necessary friction for hardware engagement.
