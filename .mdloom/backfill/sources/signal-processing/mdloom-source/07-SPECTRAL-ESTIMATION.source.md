...
proof_original: "07-SPECTRAL-ESTIMATION.md"
---
---
...
backsource_ids: [mdloom-backfill:signal-processing:07-spectral-estimation, git-history:signal-processing:07-spectral-estimation]
...
**Modern sparse spectral estimation:** MUSIC/ESPRIT require knowing p (number of sinusoids) and assume white noise. Compressed sensing approaches bypass both limitations. Atomic norm minimization (Bhaskar et al. 2013) recovers a sparse line spectrum from compressed measurements via semidefinite programming — no model order selection required. In discrete form, LASSO with a DFT dictionary (||y - Φx||₂² + λ||x||₁) achieves super-resolution by promoting sparsity in the frequency domain. These methods connect spectral estimation to the broader sparse recovery framework (see 02-SAMPLING-THEORY compressed sensing note).
