...
proof_original: "02-SAMPLING-THEORY.md"
---
---
...
backsource_ids: [mdloom-backfill:signal-processing:02-sampling-theory, git-history:signal-processing:02-sampling-theory]
...
**Compressed sensing (sub-Nyquist sampling):** Nyquist is tight for arbitrary bandlimited signals. But if the signal is **sparse** in some basis (only K out of N coefficients are nonzero), you can recover it from M ≈ O(K log(N/K)) random measurements — far fewer than N. The key conditions: the measurement matrix Φ satisfies the Restricted Isometry Property (RIP), and the sparsity basis is incoherent with the measurement basis. Recovery via L1 minimization (basis pursuit): min ||x||₁ subject to Φx = y, which is a convex program solvable by standard methods. Applications: MRI acceleration (4-8× fewer k-space samples), single-pixel camera, cognitive radio spectrum sensing. This is one of the most important results in applied mathematics since 2006 (Candès-Romberg-Tao, Donoho).
