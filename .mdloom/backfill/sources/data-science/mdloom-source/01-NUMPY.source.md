...
backsource_ids: [mdloom-backfill:data-science:01-numpy, git-history:data-science:01-numpy]
concepts: [numpy]
root_concepts: [numpy]
...
|---|---|---|
| Linear algebra operations under arrays | `mathematics/06-LINEAR-ALGEBRA.md` | ndarray shapes, dot products, decompositions, and broadcasting are linear algebra in memory |
| Numerical precision and floating point | `numerical-methods/01-FLOATING-POINT.md` | NumPy performance is useless if dtype and rounding behavior are misunderstood |
| Probability and random sampling | `probability-statistics/01-PROBABILITY-FOUNDATIONS.md` | NumPy's random module and simulations depend on probability primitives |
| ML theory on array substrates | `machine-learning-theory/01-PAC-LEARNING.md` | Training data and hypotheses become tensors, losses, and vectorized operations |
| Package/runtime environment | `computing/01-PACKAGE.md` | NumPy installs, BLAS linkage, wheels, and ABI compatibility are package-stack issues |
