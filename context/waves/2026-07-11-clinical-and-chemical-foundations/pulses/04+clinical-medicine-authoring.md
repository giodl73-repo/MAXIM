...
| Source-corpus (regenerated) | `.proof/backfill/sources/clinical-medicine/**`, `.proof/backfill/modules/clinical-medicine.json`, `.crop/views/maxim-clinical-medicine-*.json`, `.pebble/packs/maxim-clinical-medicine-*.pebble.json`, `.fletch/registries/maxim-clinical-medicine-source-corpus.json` |
...
- [x] Source-backfill with `--validate` for `clinical-medicine` (regenerated
      PROOF/CROP/PEBBLE/FLETCH) and re-validation for `medicine` (because `medicine/10`
...
```powershell
# Source-backfill regenerates PROOF/CROP/PEBBLE/FLETCH and validates
...
`--validate` passed (PROOF check, CROP inspect `--strict`, FLETCH registry validate, shaft
