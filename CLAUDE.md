...
| 2026-02-22 | 19-NUMERICAL-METHODS.md authored. IEEE 754 (float16/32/64/bfloat16, machine epsilon, subnormals, NaN propagation, catastrophic cancellation examples). Condition number κ(A) = σ_max/σ_min, digit-loss rule. Root finding: bisection (guaranteed linear), Newton-Raphson (quadratic near simple root, failure modes), Newton in ℝⁿ (Jacobian solve), secant (order φ≈1.618), Brent's (industry standard), fixed-point (Banach), convergence order table. Linear systems: Gaussian elimination + LU + Cholesky O(n³/6) for SPD, partial pivoting, special structure exploitation (banded/tridiagonal/Toeplitz/sparse). Iterative methods: Jacobi/Gauss-Seidel/SOR; CG for SPD (convergence bound √κ), GMRES, BiCGSTAB; preconditioning (ILU, AMG); multigrid O(n). Interpolation: Lagrange, Newton divided differences (barycentric O(n)), Runge's phenomenon, Chebyshev nodes (minimax), Chebyshev polynomials (Tₙ(cosθ)=cos(nθ)), cubic splines (C² piecewise, tridiagonal system), B-splines/Bezier. Least squares: normal equations (κ²), QR (stable, κ not κ²), truncated SVD (Eckart-Young), ridge regression. Finite differences: forward/central/backward, second derivative stencil, step-size dilemma (hₒₚₜ~ε^(1/3)), Richardson extrapolation (gains 2 orders). Automatic differentiation: forward vs. reverse (backprop), dual numbers, tape/Wengert list, Jacobian strategies. Quadrature: midpoint/trapezoidal/Simpson (O(h²)/O(h²)/O(h⁴)), Gaussian (2n-1 exactness), Gauss-Legendre/Hermite/Laguerre/Kronrod, adaptive (QUADPACK), MC (O(N^{-1/2}), dimension-free), QMC (Sobol/Halton), MCMC/HMC/NUTS. ODEs: Euler O(h), RK4 O(h⁴), Dormand-Prince RK45 (scipy default), stiffness (implicit Euler A-stable, Crank-Nicolson, BDF1-6, ode15s), Dahlquist A-stability. BVPs: shooting method, finite differences (tridiagonal), FEM (weak form). Eigenvalues: power iteration, Rayleigh quotient (cubic), inverse iteration, QR algorithm (Hessenberg reduction + shifts, O(n³)), Lanczos/Arnoldi for sparse. PDEs numerically: FTCS explicit heat (r≤1/2 stability), BTCS/Crank-Nicolson (unconditionally stable), CFL condition for wave, von Neumann stability analysis, FEM workflow (weak form → stiffness matrix → sparse solve), spectral methods (Fourier + Chebyshev, exponential convergence). ML numerics: GEMM, randomized SVD, condition number in training, Adam as diagonal preconditioning, sparse formats (CSR/CSC/COO), bfloat16/FP8 mixed precision. |
| 2026-02-22 | 18-NUMBER-THEORY.md authored. Divisibility + GCD (Euclidean O(log n), Bezout identity, Euclid's lemma). Unique factorization (UFD, failure in ℤ[√−5] → motivation for ideals). Primes: Euclid's theorem, sieve, PNT π(x)~x/ln x, Dirichlet's theorem on AP, Miller-Rabin (error < 4⁻ᵏ, Carmichael number problem), AKS, Pollard-ρ/QS/GNFS/Shor. Modular arithmetic: ℤ/nℤ ring, units (ℤ/nℤ)×, primitive roots, Euler's theorem + FLT, fast exponentiation. CRT (construction, ring isomorphism, RSA-CRT speedup). Arithmetic functions: φ/μ/τ/σ/λ table, multiplicativity, Dirichlet convolution (f*g), key relations (φ=μ*id, Σμ=ε), Möbius inversion, Dirichlet series + ζ(s) (Euler product, ζ(2)=π²/6, ζ(−1)=−1/12). Quadratic residues: Euler's criterion, Legendre symbol, (−1/p)/(2/p) rules, quadratic reciprocity (Gauss's golden theorem), Jacobi symbol, Tonelli-Shanks √a mod p. Diophantine: linear (Bezout), Pythagorean triples (m²−n² / 2mn / m²+n² parametrization), sums of squares (Fermat n=a²+b² iff no p≡3(4) to odd power, Lagrange 4-square, Legendre 3-square), Pell's equation (continued fractions, units in ℤ[√D]). Fermat's Last Theorem (Frey/Ribet/Wiles proof path via modularity). p-adics: vₚ, ultrametric, ℚₚ as completion, Ostrowski's theorem, adeles, Hensel's lemma. Algebraic NT: number fields, rings of integers 𝒪_K, quadratic fields, ideal class group h_K, Dirichlet's unit theorem. Crypto: RSA (setup/correctness/security/CRT), DLP (Diffie-Hellman, Pohlig-Hellman, index calculus), ECDLP (group law, Hasse-Weil, NIST/Curve25519, ECDSA nonce reuse), Post-quantum (LWE, Ring-LWE, ML-KEM/Kyber/Dilithium, LLL reduction, Coppersmith). RH (trivial/nontrivial zeros, consequences, Weil conjectures). Prime gaps (Bertrand, Zhang, Maynard-Tao, Green-Tao, Goldbach). |
| 2026-02-22 | 17-COMBINATORICS-GRAPHS.md authored. Basic counting (product/sum/bijection, P(n,r)/C(n,r), Pascal, Vandermonde, hockey-stick). Inclusion-exclusion (derangements D(n)≈n!/e, Euler φ, surjections). Pigeonhole + Erdős-Szekeres. Multinomials. Generating functions: OGFs (Fibonacci via GF, Catalan Cₙ=C(2n,n)/(n+1)), EGFs (Bell numbers, Stirling, derangements), transfer matrix method. Graph theory: definitions + handshaking, graph families (Kₙ/Kₘ,ₙ/Cₙ/Qₙ/Petersen), connectivity (κ/λ/δ, Menger, SCCs). Trees (6 equivalent conditions, Cayley nⁿ⁻² + Prüfer, Matrix-Tree theorem). Spanning trees: BFS/DFS trees, MST (Kruskal O(ElogE) + cut property, Prim O(ElogV)), matroid view. BFS/DFS algorithms + edge classification + topological sort. Shortest paths (Dijkstra/Bellman-Ford/Floyd-Warshall/Johnson/A*). Network flow: max-flow min-cut (Ford-Fulkerson, Edmonds-Karp, Dinic O(V²E)), bipartite matching (Hall's theorem, Hopcroft-Karp O(E√V)), König's theorem. Eulerian (Euler's theorem + Hierholzer) vs Hamiltonian (NP-hard, Dirac/Ore). Graph coloring (χ vs ω, Brooks, 4CT, chromatic polynomial). Planarity (Euler's formula V-E+F=2, Kuratowski/Wagner, Robertson-Seymour/treewidth). Ramsey theory: R(3,3)=6, R(5,5) open, Van der Waerden, Schur. Extremal graph theory: Turán's theorem, Mantel, forbidden subgraphs, probabilistic method (first moment, LLL, Erdős lower bound for R(k,k)). Spectral graph theory: Laplacian eigenvalues, Cheeger inequality, expander graphs. Möbius inversion on posets (→ inclusion-exclusion), Burnside/Polya. GNN connections (1-WL limit, GCN spectral view, graph transformers). DSU code, full algorithm complexity table. |
| 2026-02-22 | 16-INFORMATION-THEORY.md authored. Shannon entropy (bits/nats/trits, self-information, chain rule, concavity). Joint/conditional entropy + information Venn diagram. Mutual information (= D(P_XY‖P_X⊗P_Y), data processing inequality, sufficient statistics, InfoMax/ICA, information bottleneck). KL divergence (Gibbs/Jensen proof, forward MLE vs reverse VI/mode-seeking, f-divergence family: TV/χ²/Hellinger/JS/Wasserstein, GAN objectives). Cross-entropy = H(P) + D(P‖Q) = MLE loss. Jensen's inequality. Source coding theorem + Huffman + arithmetic coding + AEP (|A_ε| ≈ 2^nH). Channel capacity: BSC/BEC/AWGN/Shannon-Hartley (C = W log₂(1+SNR)), Fano's inequality, noisy channel coding theorem. Differential entropy (can be negative), max-entropy distributions table, Gaussian KL closed form (VAE regularizer). Fisher information + Cramér-Rao bound + natural gradient = I(θ)⁻¹∇L + K-FAC + Fisher = local KL curvature. Rate-distortion + water-filling. MDL (d/2 log n bits). Full ML connections reference map + complete VAE ELBO derivation. |
...
| 2026-02-22 | 08-PLANETARY-ATMOSPHERES.md authored. astronomy/ planetary track continuing. Atmospheric structure (troposphere/stratosphere/mesosphere/thermosphere, dry adiabatic lapse rate), scale height H=kT/mg table (Earth/Venus/Mars/Titan/Jupiter), Jeans escape parameter λ_J, non-thermal escape (ion pickup, sputtering, hydrodynamic, charge exchange), cold trap mechanism. Greenhouse effect: two-layer model, radiative forcing ΔF=5.35ln(C/C₀), runaway condition (OLR saturation ~282 W/m², Nakajima limit, Ingersoll/Kasting). Atmospheric circulation: Rossby number, Hadley/Ferrel/Polar cells, ITCZ, jet streams, Venus super-rotation (Gierasch-Rossow mechanism), Mars dust storms + ±25% seasonal pressure variation, Titan hydrocarbon cycle. Comparative table (Venus/Earth/Mars): why Venus is hot despite high albedo (absorbs less solar than Earth), Mars early climate problem (unsolved), divergence story (carbonate-silicate cycle on Earth, Venus desiccation, Mars small mass + dynamo loss). Gas giant structure (Jupiter, belt/zone, GRS, Saturn helium rain, internal heat). Biosignatures: Sagan 1993 Galileo flyby of Earth, O₂+CH₄ thermodynamic disequilibrium, abiotic O₂ false positives (M-dwarf XUV), JWST results table (WASP-39b, TRAPPIST-1b,c, K2-18b DMS disputed), vegetation red edge. |
| 2026-02-22 | 09-EXOPLANETS.md authored. Detection methods: transit (ΔF/F=(Rp/R★)², geometric probability, limb darkening, duration formula, Kepler 4-yr bias), RV (K semi-amplitude formula, M sin i degeneracy, ESPRESSO 0.3 m/s precision, stellar activity), direct imaging (contrast 10⁻¹⁰ for Earth, current 10⁻⁷ limit, HR 8799, β Pic b), microlensing (Einstein radius, Roman survey, no repeats), astrometry (Gaia DR3/DR4). Demographics: occurrence rates, sub-Neptune dominance, hot Jupiter rarity. Fulton gap (~1.7 R_E, bimodal, CKS 2017): photoevaporation model (Owen & Wu) vs core-powered mass loss (Ginzburg). M-R diagram families (iron/rocky/water world/gas dwarf) + EOS table + degeneracy problem + JWST 55 Cnc e. Habitable zone: classical (Kasting/Kopparapu 0.99–1.68 AU for Sun), moist greenhouse, maximum greenhouse, H₂ extended HZ, subsurface ocean HZ. TRAPPIST-1 full table (7 planets, masses from TTV Agol 2021, tidal locking, resonance chain, XUV concern, JWST results). Hot Jupiters: disk migration vs high-e (Kozai-Lidov + tidal) pathways, spin-orbit misalignment, ultra-hot Jupiters (KELT-9b 4000K, iron vapor), magnetic inflation. Future missions: PLATO 2026, Roman 2027, HWO ~2040. K2-18b Hycean world controversy. |
| 2026-02-22 | 10-SMALL-BODIES.md authored. Asteroid belt: Kirkwood gaps (3:1/5:2/7:3/2:1 MMRs), Hilda 3:2 stable, Jupiter Trojans (Lucy mission), composition gradient (S-type inner, C-type outer, P/D outer-system implants), Yarkovsky da/dt formula + YORP spin-up → binaries. NEOs: Amor/Apollo/Aten/Atira classification, PHAs (MOID<0.05 AU), Torino scale, Apophis 2029 flyby (32,000 km). DART 2022: kinetic impactor → Dimorphos period change 33 min, β≈3.6 ejecta amplification, Hera follow-up. Deflection method comparison table. Comets: nucleus structure (icy dirtball, ρ~0.5 g/cm³, 70-80% porosity), Rosetta/67P (bilobed, D/H=3×SMOW), 2I/Borisov (interstellar, CO-rich), coma/ion tail/dust tail anatomy, JFC/Halley-type/long-period classification (Tisserand parameter). Kuiper Belt: cold classical (binary-rich, red, Arrokoth contact binary), hot classical (Nice model scattered), resonant Plutinos (3:2), SDOs (JFC feeder), detached (Sedna), Planet Nine hypothesis (clustering of extreme TNOs, Vera Rubin LSST test). Oort Cloud: structure (10¹² objects, 2000–100,000 AU), origin (Öpik-Oort model, Jupiter scattered), injection mechanisms (galactic tide dominant, stellar encounters), possible ~63 Myr comet shower periodicity. Dwarf planets: Pluto (New Horizons: N₂ plains, CH₄ mountains, haze layers, LMO Charon Mordor Macula, mutual tidal lock), Eris/Makemake/Quaoar (ring beyond Roche — elastic material hypothesis)/Gonggong/Ceres (Dawn: briny Occator deposits, active geology). Impacts: energy table (25m=Tunguska/140m=GT/1km=10⁵ GT/10km=extinction), Chicxulub (10-15km, 180km crater, Ir anomaly, Alvarez 1980, ~75% extinction, Deccan Traps coincidence), Tunguska 1908, Chelyabinsk 2013. Meteorite taxonomy: chondrites (CI/CM/CV/H/L/LL/E), achondrites (HED=Vesta, SNC=Mars, lunar, irons), CAIs (4.5672 Gyr), chondrules (4.565 Gyr, flash-heating debate), sample return table (Hayabusa/Hayabusa2 Ryugu CM-like/OSIRIS-REx Bennu 121g). |
| 2026-02-22 | 11-ASTROBIOLOGY.md authored. astronomy/ planetary sciences track COMPLETE (11/11). Abiogenesis: chicken-and-egg problem, RNA world hypothesis (Gilbert 1986, ribosome as ribozyme, coenzyme nucleotide fossils, Sutherland UV/HCN/H₂S synthesis), alkaline hydrothermal vent model (Russell/Martin, Lost City, proton gradient → Mitchell chemiosmosis, Fe-S micropore compartmentalization), Miller-Urey 1953 (amino acids from reducing atm, revised significance for volcanic/vent reducing pockets), LUCA (3.8-4.0 Gya, thermophilic, chemolithotrophic, H₂-oxidizing). Requirements: liquid water (polarity/amphoteric/heat capacity/ice floats), CHNOPS + metals, energy gradient, thermodynamic disequilibrium; life-as-we-don't-know-it (Si/NH₃/methane alternatives). Extremophiles table: thermophiles/hyperthermophiles (122°C)/psychrophiles/halophiles/acidophiles (pH 0)/alkaliphiles (pH 12)/barophiles (110 MPa)/radioresistant (D. radiodurans 17 kGy)/anaerobes. Solar system targets: Europa (Galileo magnetometer induced field, chaos terrain, 100km ocean, Europa Clipper 2030), Enceladus (Cassini H₂+organics+alkaline pH+silica nanoparticles, methanogenesis energy equation, VERY HIGH habitability), Mars (Jezero delta confirmed, InSight/MARSIS subsurface brine, methane controversy), Titan (Dragonfly 2034). Biosignatures: Lovelock 1965 disequilibrium, Sagan 1993 Galileo, O₂+CH₄ pair, O₃ proxy, N₂O, DMS, false positive gauntlet table, VRE, technosignatures (narrowband radio/laser/NO₂/waste heat). JWST K2-18b DMS controversy (3σ, Hycean or magma ocean), TRAPPIST-1 priority (100-200 hrs/planet). Fermi paradox: paradox framing (colonization timescale << galaxy age), Drake equation table, resolutions (Great Filter optimist/pessimist, Rare Earth, GOE 2-Gyr delay, Zoo, Dark Forest, self-destruction, Transcension). SETI history: Ozma 1960, Wow! 1977, Arecibo message 1974, Breakthrough Listen, BLC1, FAST. Panspermia: lithopanspermia plausibility (Mars→Earth 10⁴ yr, spore survival, SNC meteorites confirm transfer), directed panspermia (Crick 1973, now disfavored). |
| 2026-02-22 | Session 7 complete. natural-sciences/ series: 17 files. 00-OVERVIEW (scale hierarchy subatomic→cosmological, field taxonomy tree, 6 unifying principles, cross-disciplinary hybrids, math toolkit by domain). Chemistry: 01-ATOMIC-QUANTUM (H atom exact solution, quantum numbers, Zeff/shielding, Madelung rule, periodic trends, spectroscopy, relativistic effects, computation ladder HF→DFT→CCSD(T)), 02-BONDING (ionic/covalent/metallic/weak interactions spectrum, VSEPR full table, MO theory + Period 2 diatomics, flux freezing, band theory, London/H-bond), 03-THERMOCHEM (four laws, ΔH/ΔG/ΔS, Hess, Kirchhoff, Clausius-Clapeyron, phase diagrams, colligative, Carnot, equilibrium + Le Chatelier + Van't Hoff), 04-KINETICS (integrated rate laws, Arrhenius, TST/Eyring, SSA, pre-equilibrium, catalysis, chain reactions), 05-ELECTROCHEMISTRY (redox balancing, galvanic/electrolytic cells, standard potentials, Nernst, Pourbaix, Butler-Volmer, batteries, corrosion). Biochemistry: 06-BIOMOLECULES (20 AA side-chain chemistry/pKa, protein structure primary→quaternary, Ramachandran, α-helix/β-sheet, nucleic acids, lipid bilayer, carbohydrates), 07-ENZYMES (M-M derivation, kcat/Km, Lineweaver-Burk, competitive/uncompetitive/noncompetitive inhibition, Hill/MWC allostery, hemoglobin Bohr effect, zymogens, EC classes), 08-METABOLISM (glycolysis 10 steps + regulation, PDC, TCA cycle, ETC complexes I-V, chemiosmosis, ~30 ATP accounting, gluconeogenesis, β-oxidation, PPP, energy charge), 09-MOLECULAR-BIO (replisome, telomerase, prokaryotic/eukaryotic transcription, pre-mRNA processing, spliceosome, translation mechanism, lac operon, eukaryotic enhancers/chromatin, CRISPR-Cas9, central dogma exceptions). Biology: 10-CELL-BIOLOGY (organelle map, membrane transport all types, GPCR/RTK/MAPK/PI3K-AKT-mTOR signaling, cytoskeleton + motor proteins, cell cycle checkpoints + SAC + CDK-cyclin, apoptosis intrinsic/extrinsic), 11-EVOLUTION-GENETICS (Mendelian laws + extensions, mutation types + repair pathways, Hardy-Weinberg, selection/drift/Wright-Fisher/coalescent, dN/dS, phylogenetics 4 methods, speciation mechanisms, evo-devo + Hox genes), 12-SYSTEMS-SYNTHETIC (GRN Boolean/ODE models, network motifs, FBA, noise/Fano factor, synthetic circuit design principles, toggle switch, repressilator, CRISPRi/a, optogenetics). Earth sciences: 13-GEOPHYSICS (Earth interior layers + seismic evidence, P/S shadow zones, focal mechanisms, mantle convection + Rayleigh number, plate boundaries + Wilson cycle, paleomagnetism, geodynamo + reversals, isostasy Airy/Pratt), 14-ATMOSPHERE-CLIMATE (atmospheric layers, radiation balance + greenhouse effect, CO₂ forcing logarithmic relation, ozone chemistry + Montreal Protocol, Hadley/Ferrel/Polar cells, AMOC/thermohaline, ENSO, climate sensitivity ECS~3°C, feedbacks, GCM architecture). Plasma: 15-PLASMA-FUNDAMENTALS (Debye shielding derivation, plasma parameter, plasma frequency, Larmor radius, guiding center drifts E×B/gradient/curvature, magnetic mirror + loss cone, ideal/resistive MHD, flux freezing, wave taxonomy Alfvén/Langmuir/whistler, Landau damping), 16-PLASMA-DYNAMICS (MHD equilibrium, kink/RT instabilities, Kruskal-Shafranov criterion, tearing modes, magnetic reconnection Sweet-Parker/Petschek/plasmoid, tokamak geometry + heating + Lawson criterion, stellarator W7-X, ICF + NIF 2022 ignition, solar wind/magnetosphere, CMEs, accretion disk MRI, Hall thruster + industrial applications). |
...
| `mathematics/16-INFORMATION-THEORY.md` | Shannon entropy, mutual information, KL divergence, channel capacity, ML bridges | ✅ Complete |
| `mathematics/17-COMBINATORICS-GRAPHS.md` | Counting, generating functions, graph theory, trees, planarity, Ramsey, extremal | ✅ Complete |
| `mathematics/18-NUMBER-THEORY.md` | Primes, modular arithmetic, Euler's theorem, quadratic reciprocity, p-adics, crypto | ✅ Complete |
| `mathematics/19-NUMERICAL-METHODS.md` | Newton's method, quadrature, finite differences, stability, condition numbers | ✅ Complete |
| `mathematics/20-STATISTICS.md` | Estimation, MLE, hypothesis testing, Bayesian inference, regression theory | 🔜 Next |
...
| File | Topic | Status |
|------|-------|--------|
| `natural-sciences/00-OVERVIEW.md` | Field taxonomy, scale hierarchy, unifying principles, cross-discipline bridges | ✅ Complete |
| `natural-sciences/01-ATOMIC-QUANTUM.md` | Atomic structure, quantum numbers, orbitals, periodic trends, spectroscopy | ✅ Complete |
| `natural-sciences/02-BONDING.md` | Ionic/covalent/metallic bonding, VSEPR, MO theory, hybridization, weak interactions | ✅ Complete |
| `natural-sciences/03-THERMOCHEM.md` | Enthalpy, entropy, Gibbs free energy, Hess's law, equilibrium, phase diagrams | ✅ Complete |
| `natural-sciences/04-KINETICS.md` | Rate laws, Arrhenius, transition state theory, mechanisms, catalysis, chain reactions | ✅ Complete |
| `natural-sciences/05-ELECTROCHEMISTRY.md` | Redox, electrochemical cells, Nernst equation, Pourbaix diagrams, batteries, corrosion | ✅ Complete |
| `natural-sciences/06-BIOMOLECULES.md` | Amino acids, proteins (primary→quaternary), nucleic acids, lipids, carbohydrates | ✅ Complete |
| `natural-sciences/07-ENZYMES.md` | Michaelis-Menten, inhibition types, allostery, Hill equation, covalent modification | ✅ Complete |
| `natural-sciences/08-METABOLISM.md` | Glycolysis, TCA cycle, oxidative phosphorylation, ATP accounting, gluconeogenesis, β-oxidation | ✅ Complete |
| `natural-sciences/09-MOLECULAR-BIO.md` | DNA replication, transcription, pre-mRNA processing, translation, gene regulation, CRISPR | ✅ Complete |
| `natural-sciences/10-CELL-BIOLOGY.md` | Organelles, membrane transport, signal transduction (GPCR/RTK/MAPK), cytoskeleton, cell cycle, apoptosis | ✅ Complete |
| `natural-sciences/11-EVOLUTION-GENETICS.md` | Mendelian genetics, Hardy-Weinberg, population genetics, molecular evolution, phylogenetics, speciation, evo-devo | ✅ Complete |
| `natural-sciences/12-SYSTEMS-SYNTHETIC.md` | GRNs, Boolean/ODE models, FBA, network motifs, synthetic circuits (toggle, repressilator), optogenetics, CRISPRi/a | ✅ Complete |
| `natural-sciences/13-GEOPHYSICS.md` | Earth interior, seismology, mantle convection, plate tectonics, geomagnetism, isostasy | ✅ Complete |
| `natural-sciences/14-ATMOSPHERE-CLIMATE.md` | Atmospheric layers, radiation balance, greenhouse effect, global circulation, ENSO, climate feedbacks, GCMs | ✅ Complete |
| `natural-sciences/15-PLASMA-FUNDAMENTALS.md` | Debye shielding, plasma frequency, cyclotron motion, guiding center drifts, MHD equations, waves | ✅ Complete |
| `natural-sciences/16-PLASMA-DYNAMICS.md` | MHD instabilities, magnetic reconnection, tokamaks, stellarators, ICF, astrophysical plasmas, industrial | ✅ Complete |
