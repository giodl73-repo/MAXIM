...
2. **NEVER glob-process files outside the target directory.** If working on `atlas/`, do not glob `reference/*.md`. Scope all file operations to the exact directory being modified.
...
- **PUZZLE ANSWER PROTOCOL**: Puzzle answers are encoded in `memory/k53.md` using A1Z26→periodic table element symbols (A=H, B=He, C=Li...). NEVER write plaintext puzzle answers to files in the repo. When authoring puzzles, answers are embedded in puzzle mechanics (the DNA decodes to the answer, the grid solves to the answer) — that's fine. But never create a plaintext answer key file. The `joker/` puzzle files should NOT have explicit `**Answer: WORD**` lines — strip those during final assembly. Encoded answers in `.claude/` memory are the only master reference.
