...
<!-- @editor[bridge/P2]: The Hall-Petch relation (σ = σ₀ + k/√d) is mentioned here only in Common Confusion Points. But it belongs earlier in the main body — it is the key quantitative bridge between microstructure (grain size d) and property (yield strength σ): yield strength scales as the inverse square root of grain size. This is the same scaling relationship found in percolation theory and random networks (critical threshold scales with system size). The √d denominator arises from dislocation pile-up theory — same physics as stress concentration at crack tips (K = σ√(πa)). The connection to the fracture mechanics scaling (05-FRACTURE-MECHANICS.md) would reinforce both guides. -->
