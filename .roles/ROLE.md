# MAXIM Role Index

MAXIM is a large reference library. Use these roles when changing guide
structure, graduation criteria, public navigation, or reader-facing content.

## Parliament

| File | Role | Primary tension |
|---|---|---|
| `parliament/reference-integrity-auditor.md` | Reference Integrity Auditor | Breadth and synthesis vs. factual precision |

## Editorial

| File | Role | Primary tension |
|---|---|---|
| `editorial/reader-path-editor.md` | Reader Path Editor | Complete coverage vs. usable learning paths |

## Stakeholders

| File | Role | Primary tension |
|---|---|---|
| `stakeholders/learner-advocate.md` | Learner Advocate | Expert compression vs. reader orientation |

## Review order

1. Use the Reader Path Editor when navigation, routes, or guide structure change.
2. Use the Reference Integrity Auditor when facts, scope, or synthesis change.
3. Use the Learner Advocate before graduating guides or public-facing bundles.
