...
<!-- @editor[bridge/P2]: No computational MHD bridge — the MHD equations are notoriously hard to solve numerically. The guide gives the physics and dimensionless numbers but not the numerical landscape: spectral methods (Fourier basis for periodic domains, used in turbulence simulations), lattice Boltzmann methods (LBM for fluid + BGK collision operator), and the specific challenge of maintaining ∇·B = 0 numerically (constrained transport, divergence cleaning). A reader coming from TCS will ask "how do you simulate this?" and find no answer. Even a paragraph on the computational hardness (explicit CFL condition, stiff resistive terms at low Rm) would serve this reader. -->
...
<!-- @editor[bridge/P2]: Missing magnetic helicity / topology bridge — magnetic helicity H = ∫A·B dV is a topologically conserved quantity in ideal MHD (analogous to linking number of field lines). Helicity conservation constrains dynamo dynamics and reconnection. This is the bridge from MHD to topological field theory — relevant to both the math background and the quantum gravity / topological phases context that runs through the later modules. A paragraph on helicity and what it measures (writhe + twist of field lines) would elevate this guide for this reader. -->
