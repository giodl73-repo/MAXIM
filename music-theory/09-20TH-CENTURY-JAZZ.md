...
<!-- @editor[bridge/P2]: The four row forms (P, I, R, RI) × 12 transpositions constitute a group action of the dihedral group D₁₂ (or more precisely, the group generated by inversion and retrograde operations composed with transpositions) acting on the set of all 12-tone rows derived from P₀. The group has order 48 (= 4 × 12). The P/I/R/RI operations are: P = identity; I = pitch-class inversion (negate all intervals mod 12); R = time reversal; RI = both. These generate a group isomorphic to Z₁₂ × Z₂ × Z₂ (transposition × inversion × retrograde). The 4×4 matrix of row forms is literally a Cayley table for a subgroup of this structure. Webern was obsessed with rows that had special symmetry properties (palindrome = row equals its own retrograde at some transposition level; row = its own inversion, etc.) — these are exactly the fixed-point subsets of the group action, i.e., rows with non-trivial stabilizer subgroups. Naming the group theory once makes the whole structure transparent. -->
...
<!-- @editor[bridge/P1]: Reich's phase music is a direct implementation of comb filtering in the time domain. Two identical periodic signals with a gradually increasing phase offset produce constructive and destructive interference at each point in the pattern — exactly the behavior of a comb filter (sum of a signal with a delayed copy). As the phase difference increases from 0 to one full pattern period, the superposition cycles through all possible interference states. "Clapping Music" makes this explicit: one performer stays on the original pattern, the other shifts by integer-multiple sixteenth notes — this is discrete phase shifts through 12 states, and the composite pattern at each step is the convolution of the original pattern with a delta at the phase offset position. This is not an analogy — it IS signal processing, applied to rhythm. Stravinsky's "The Rite of Spring" metric complexity and Conlon Nancarrow's player piano pieces (rational tempo ratios) also fit this framework. Any signal processing or DSP-background reader will recognize this immediately. -->
